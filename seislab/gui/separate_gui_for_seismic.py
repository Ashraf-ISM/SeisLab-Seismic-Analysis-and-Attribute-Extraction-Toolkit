"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   SeismoForge  –  Seismic Interpretation Platform  (Light Theme)       ║
...
def _read_trace_headers(filepath, limit=500):
    if not SEGYIO:
        return []

    def _field_value(header, field):
        try:
            return header[field]
        except Exception:
            return ""
    ...
    with segyio.open(filepath, ignore_geometry=True) as segy_file:
        for i, header in enumerate(segy_file.header):
            ...
            headers.append({
                "Trace #": i + 1,
                "CDP": _field_value(header, segyio.TraceField.CDP),
                "Inline": _field_value(header, segyio.TraceField.INLINE_3D),
                "Crossline": _field_value(header, segyio.TraceField.CROSSLINE_3D),
                "Offset": _field_value(header, segyio.TraceField.offset),
                "ShotX": _field_value(header, segyio.TraceField.SourceX),
                "ShotY": _field_value(header, segyio.TraceField.SourceY),
                "ScalarElev": _field_value(header, segyio.TraceField.ElevationScalar),
            })
    return headers


def cube_from_loader(loader: SegyLoader) -> SeismicCube:
    cube = SeismicCube()
    cube.filename = os.path.basename(loader.filepath)
    cube.filepath = loader.filepath
    cube.loaded_at = datetime.datetime.now().strftime("%Y-%m-%d  %H:%M:%S")
    cube.source_format = str(getattr(loader, "source_format", "SEGY"))

    data = np.asarray(loader.data, dtype=np.float32)
    if data.ndim == 2:
        data = data[:, np.newaxis, :]

    cube.cube = data
    cube.n_inline, cube.n_crossline, cube.n_time = data.shape
    cube.sample_rate_ms = float(getattr(loader, "sample_rate", 1.0) or 1.0)

    info = build_loader_info(loader)
    geometry = dict(getattr(loader, "geometry_info", {}) or {})
    selected_pair = geometry.get("selected_pair", ("Inline", "Crossline"))
    cube.file_headers = {
        "Survey File": cube.filename,
        "Format": cube.source_format,
        "Mode": str(info.get("mode", "unknown")).upper(),
        "Inlines": cube.n_inline,
        "Crosslines": cube.n_crossline,
        "Time Samples": cube.n_time,
        "Sample Rate": f"{cube.sample_rate_ms:.3f} ms",
        "Selected Headers": f"{selected_pair[0]} × {selected_pair[1]}",
        "Geometry Source": geometry.get("source", "unknown"),
        "Load Report": getattr(loader, "load_report", "No load report available."),
    }
    binary_header = getattr(loader, "binary_header", {})
    if isinstance(binary_header, dict):
        cube.binary_header = binary_header
    elif binary_header:
        cube.binary_header = {"Binary Header": str(binary_header)}

    cube.text_header = str(getattr(loader, "text_header", "") or "")
    cube.load_report = str(getattr(loader, "load_report", "") or "")

    if cube.source_format == "SEGY":
        try:
            cube.trace_headers = _read_trace_headers(loader.filepath)
        except Exception:
            cube.trace_headers = []
    return cube
...
class CubeLoader(QThread):
    ...
    def run(self):
        try:
            self.progress.emit(10, "Opening seismic survey…")
            loader = SegyLoader(self.filepath)
            self.progress.emit(40, "Reading volume data…")
            loader.load_data()
            self.progress.emit(80, "Building seismic workspace…")
            cube = cube_from_loader(loader)
...
class RibbonWidget(QWidget):
    ...
    def show_tab_titles(self, visible: bool):
        self._bar.setVisible(visible)
        self.setFixedHeight(94 if visible else 68)
...
# ═══════════════════════════════════════════════════════════════════════
class InterpretationWorkspace(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        title = QLabel("Interpretation Workspace")
        title.setStyleSheet(
            f"color:{C['text_primary']};font-size:14px;font-weight:700;"
        )
        subtitle = QLabel(
            "Horizon picking and seismic attribute analysis stay together here. "
            "Menu actions under Interpretation open these tools directly."
        )
        ...
        self.tabs.tabBar().setObjectName("panelBar")

        self.interpretation_panel = InterpretationPanel()
        self.attribute_panel = AttributePanel()

        self.tabs.addTab(self.interpretation_panel, "Horizon Picking")
    ...
    def set_section(
        ...
        *,
        ...
        self.interpretation_panel.set_data(section_data)
    ...
    def show_interpretation(self):
        self.tabs.setCurrentWidget(self.interpretation_panel)

    def show_attributes(self):
        self.tabs.setCurrentWidget(self.attribute_panel)

    def compute_attribute(self, attribute_name):
        if self._section_data is None:
            QMessageBox.warning(self, "No Data", "Load seismic data before computing attributes.")
            return
        self.show_attributes()
        self.attribute_panel.compute_attribute(
            attribute_name,
            self._section_data,
            sample_rate_ms=self._section_meta.get("sample_rate_ms", 2.0),
            sample_axis=1,
        )
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("SeismoForge  –  Seismic Interpretation Platform")
        ...
        self._build_right()
        self._build_status()
        self._update_right_panel_visibility(0)
    ...
    def _build_title_ribbon(self):
        ...
        tbl.addWidget(ico)
        self._proj_lbl = QLabel("SeismoForge  –  [Synthetic_Survey_3D]")
        self._proj_lbl.setStyleSheet(f"color:{C['text_primary']};font-size:12px;font-weight:600;")
        tbl.addWidget(self._proj_lbl)
        tbl.addStretch()

        for ch, tip, cb in [("📂","Open Survey",self._open_file),
                              ("💾","Save",lambda:None),
        ...
        hs = h.section("File")
        hs.add("📂","Open Survey",big=True).clicked.connect(self._open_file)
        ...
        s2.add("✎","Horizons",big=True).clicked.connect(self._open_interpretation_workspace)
        s2.add("🧭","Workspace",big=True).clicked.connect(self._open_interpretation_workspace)
        s2.add_sep()
        s3 = si.section("Attributes")
        s3.add("⚡","Studio",big=True).clicked.connect(self._show_attribute_studio)
        s3.add("∿","RMS",big=True).clicked.connect(lambda: self._compute_attribute("rms"))
        s3.add("◔","Inst. Amp",big=True).clicked.connect(lambda: self._compute_attribute("inst_amp"))
        s3.add_sep()
        ...
        # Well Logs
        wl = self._ribbon.add("Well Logs")
        wl1 = wl.section("Well Log Panel")
        wl1.add("🪵","Open Panel",big=True).clicked.connect(self._open_well_logs)
        wl1.add("📥","Import LAS",big=True).clicked.connect(lambda: self._open_well_logs(import_las=True))
        wl1.add_sep()
        wl2 = wl.section("Window")
        wl2.add("📊","Bring Front",big=True).clicked.connect(self._open_well_logs)
        ...
        self._ribbon.show_tab_titles(False)
        ...
        file_menu.addAction("Open Survey…", self._open_file, "Ctrl+O")
        file_menu.addAction("Load Demo Data", self._load_demo)
        file_menu.addSeparator()
        file_menu.addAction("Data Information…", self._show_data_info, "Ctrl+I")
        file_menu.addSeparator()
        file_menu.addAction("Export Figure…", self._export_fig, "Ctrl+E")
        file_menu.addSeparator()
        file_menu.addAction("Quit", self.close, "Ctrl+Q")

        home_menu = mb.addMenu("Home")
        home_menu.addAction("Open in Browser", lambda: self._vp3d.open_browser())
        home_menu.addAction("Render 3D Scene", self._render_3d)

        interp_menu = mb.addMenu("Interpretation")
        interp_menu.addAction("Open Interpretation Workspace", self._open_interpretation_workspace, "Ctrl+Shift+I")
        interp_menu.addAction("Horizon Picking", self._open_interpretation_workspace)
        attr_menu = interp_menu.addMenu("Attributes")
        attr_menu.addAction("Attribute Studio", self._show_attribute_studio, "Ctrl+Shift+A")
        attr_menu.addSeparator()
        attr_menu.addAction("RMS Amplitude", lambda: self._compute_attribute("rms"))
        attr_menu.addAction("Instantaneous Amplitude", lambda: self._compute_attribute("inst_amp"))
        attr_menu.addAction("Instantaneous Phase", lambda: self._compute_attribute("inst_phase"))
        attr_menu.addAction("Instantaneous Frequency", lambda: self._compute_attribute("inst_freq"))

        view3d_menu = mb.addMenu("3D")
        view3d_menu.addAction("Reset to 3D View", lambda: self._vp_tabs.setCurrentWidget(self._vp3d))
        view3d_menu.addAction("Open 3D in Browser", lambda: self._vp3d.open_browser())
        view3d_menu.addAction("Export 3D Figure…", self._export_fig)

        well_menu = mb.addMenu("Well Logs")
        well_menu.addAction("Open Well Log Panel", self._open_well_logs, "Ctrl+Shift+W")
        well_menu.addAction("Import LAS…", lambda: self._open_well_logs(import_las=True))

        window_menu = mb.addMenu("Window")
        window_menu.addAction("Open Data Information", self._show_data_info)
        window_menu.addAction("Open Attribute Studio", self._show_attribute_studio)

        help_menu = mb.addMenu("Help")
        help_menu.addAction("About SeismoForge", self._about)

        home_menu.aboutToShow.connect(lambda: self._ribbon.select(0))
        interp_menu.aboutToShow.connect(lambda: self._ribbon.select(1))
        view3d_menu.aboutToShow.connect(lambda: self._ribbon.select(2))
        well_menu.aboutToShow.connect(lambda: self._ribbon.select(3))
        window_menu.aboutToShow.connect(lambda: self._ribbon.select(4))
        cv.addWidget(mb)
    ...
    def _build_central(self):
        ...
        self._interp_workspace = InterpretationWorkspace()
        self._vp_tabs.addTab(self._interp_workspace, "🧭  Interpretation Workspace")

        for idx in range(self._vp_tabs.count()):
            self._vp_tabs.tabBar().setTabButton(idx, QTabBar.RightSide, None)
    ...
    def _update_right_panel_visibility(self, idx=None):
        if idx is None:
            idx = self._vp_tabs.currentIndex() if hasattr(self, "_vp_tabs") else 0
        if hasattr(self, "_right_dock"):
            self._right_dock.setVisible(idx == 0)
    ...
    def _open_file(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            "Open Seismic Survey",
            "",
            "Seismic Files (*.segy *.sgy *.seg *.SGY *.SEGY *.nc *.nc4 *.cdf *.netcdf);;"
            "SEG-Y Files (*.segy *.sgy *.seg *.SGY *.SEGY);;"
            "NetCDF Files (*.nc *.nc4 *.cdf *.netcdf);;All (*.*)",
        )
    ...
    def _on_loaded(self, cube):
        ...
        self._proj_lbl.setText(f"SeismoForge  –  [{cube.filename}]")
    ...
    def _load_demo(self):
        ...
        self._proj_lbl.setText(f"SeismoForge  –  [{self.cube.filename}]")
    ...
    def _open_well_logs(self, import_las=False):
        try:
            ...
            QMessageBox.warning(
                self,
                "Well Logs",
                f"Well log panel could not be opened.\n\n{exc}",
            )
            return

        if self._well_window is None:
            self._well_window = WellDataWindow(self)
            if hasattr(self._well_window, "well_loaded"):
                self._well_window.well_loaded.connect(self._on_well_log_loaded)

        if self._well_window.isMinimized():
            self._well_window.showNormal()
        self._well_window.show()
        self._well_window.raise_()
        self._well_window.activateWindow()

        if import_las:
            self._well_window.open_las_dialog()

    def _on_well_log_loaded(self, filepath):
        self._lbl_status.setText(f"Loaded well log: {os.path.basename(filepath)}")

    def _current_section_for_interpretation(self):
        if not self.cube:
            return None, None

        p = self._props
        tab_index = self._vp_tabs.currentIndex()

        if tab_index == 2:
            section = self.cube.cube[:, p.xl_idx, :]
            meta = {
                "section_kind": "Crossline",
                "section_index": p.xl_idx,
                "horizontal_label": "Inline",
                "vertical_label": "Time (ms)",
            }
        else:
            section = self.cube.cube[p.il_idx, :, :]
            meta = {
                "section_kind": "Inline",
                "section_index": p.il_idx,
                "horizontal_label": "Crossline",
                "vertical_label": "Time (ms)",
            }

        meta.update({
            "sample_rate_ms": self.cube.sample_rate_ms,
            "volume": self.cube.cube,
            "inline_index": p.il_idx,
            "crossline_index": p.xl_idx,
            "timeslice_index": p.t_idx,
        })
        return section, meta

    def _sync_interpretation_workspace(self):
        if not self.cube:
            return
        section, meta = self._current_section_for_interpretation()
        if section is None:
            return
        self._interp_workspace.set_section(section, **meta)

    def _open_interpretation_workspace(self):
        if not self.cube:
            QMessageBox.information(self, "No Data", "Load a seismic survey first.")
            return
        self._sync_interpretation_workspace()
        self._vp_tabs.setCurrentWidget(self._interp_workspace)
        self._interp_workspace.show_interpretation()
        self._lbl_status.setText("Interpretation workspace ready.")

    def _show_attribute_studio(self):
        if not self.cube:
            QMessageBox.information(self, "No Data", "Load a seismic survey first.")
            return
        self._sync_interpretation_workspace()
        self._vp_tabs.setCurrentWidget(self._interp_workspace)
        self._interp_workspace.show_attributes()
        self._lbl_status.setText("Attribute studio ready.")

    def _compute_attribute(self, attribute_name):
        if not self.cube:
            QMessageBox.information(self, "No Data", "Load a seismic survey first.")
            return
        self._sync_interpretation_workspace()
        self._vp_tabs.setCurrentWidget(self._interp_workspace)
        self._interp_workspace.compute_attribute(attribute_name)
        self._lbl_status.setText(f"Computing {attribute_name} on the active interpretation section…")
    ...
    def _render_current(self):
        {0:self._render_3d,1:self._render_il,2:self._render_xl,3:self._render_ts,
         4:self._sync_interpretation_workspace
         }.get(self._vp_tabs.currentIndex(),lambda:None)()

    def _on_tab(self, idx):
        self._update_right_panel_visibility(idx)
        if self.cube:
            {0:self._render_3d,1:self._render_il,2:self._render_xl,3:self._render_ts,
             4:self._sync_interpretation_workspace
             }.get(idx,lambda:None)()
    ...
    def _about(self):
        QMessageBox.about(self,"About SeismoForge",
            f"<h2 style='color:{C['accent']}'>SeismoForge</h2>"
...
def main():
    ...
    app.setApplicationName("SeismoForge")
    app.setStyle("Fusion")
